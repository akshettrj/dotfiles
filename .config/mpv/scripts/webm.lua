...
local options = {
	...
	strict_bitrate_multiplier = 0.25,
	-- In kilobits.
...
expand_properties = function(text, magic)
	...
	for prefix, raw, prop, colon, fallback, closing in text:gmatch("%" .. magic .. "{([?!]?)(=?)([^}:]*)(:?)([^}]*)(}*)}") do
		...
		if colon == ":" then
			...
			text, _ = text:gsub("%" ..
				magic ..
				"{" .. prefix .. raw .. original_prop:gsub("%W", "%%%1") .. ":" .. fallback:gsub("%W", "%%%1") .. closing .. "}",
				expand_properties(prop_value))
		else
			local _
			text, _ = text:gsub("%" .. magic .. "{" .. prefix .. raw .. original_prop:gsub("%W", "%%%1") .. closing .. "}",
				prop_value)
		end
	end
	return text
...
format_filename = function(startTime, endTime, videoFormat)
	...
	if mp.get_property_bool("demuxer-via-network", false) then
		...
		local x = string.gsub(mp.get_property("stream-open-filename", ""),
...
get_video_dimensions = function()
	...
	if keep_aspect then
		...
		_video_dimensions.top_left.x, _video_dimensions.bottom_right.x = split_scaling(window_w, scaled_width, zoom, align_x,
			pan_x)
		local align_y = mp.get_property_number("video-align-y")
		local pan_y = mp.get_property_number("video-pan-y")
		_video_dimensions.top_left.y, _video_dimensions.bottom_right.y = split_scaling(window_h, scaled_height, zoom, align_y,
			pan_y)
	else
	...
	local _base_0 = {
		postCommandModifier = function(self, command, region, startTime, endTime)
			...
			for _, v in ipairs(command) do
				...
				repeat
					if v:match("^%-%-vf%-add=lavfi%-scale") or v:match("^%-%-vf%-add=lavfi%-crop") or v:match("^%-%-vf%-add=fps") or
						v:match("^%-%-vf%-add=lavfi%-eq") then
			...
			if options.gif_dither == 6 then
				...
				cfilter = cfilter ..
					"[vidf][pal]paletteuse=dither=bayer:bayer_scale=" .. tostring(options.gif_dither) .. ":diff_mode=rectangle[vo]"
			end
			append(new_command, {
...
get_contrast_brightness_and_saturation_filters = function()
	...
	return {
		"lavfi-eq=contrast=" ..
			tostring(eq_contrast) .. ":saturation=" .. tostring(eq_saturation) .. ":brightness=" .. tostring(eq_brightness)
	}
end
...
get_video_filters = function(format, region)
	...
	if region and region:is_valid() then
		append(filters, {
			"lavfi-crop=" ..
				tostring(region.w) .. ":" .. tostring(region.h) .. ":" .. tostring(region.x) .. ":" .. tostring(region.y)
		})
	end
...
do
	...
	local _base_0 = {
		...
		draw = function(self)
			...
			ass:append(tostring(bold('1:')) ..
				" change point A (" .. tostring(self.pointA.x) .. ", " .. tostring(self.pointA.y) .. ")\\N")
			ass:append(tostring(bold('2:')) ..
				" change point B (" .. tostring(self.pointB.x) .. ", " .. tostring(self.pointB.y) .. ")\\N")
	...
	local _base_0 = {
		...
		setValue = function(self, value)
			...
			elseif "list" == _exp_0 then
				...
				if not set then
					return msg.warn("Tried to set invalid value " ..
...
do
	...
	local _base_0 = {
		...
		draw = function(self)
			...
			ass:append(tostring(bold('1:')) ..
				" set start time (current is " .. tostring(seconds_to_time_string(self.startTime)) .. ")\\N")
			ass:append(tostring(bold('2:')) ..
				" set end time (current is " .. tostring(seconds_to_time_string(self.endTime)) .. ")\\N")
